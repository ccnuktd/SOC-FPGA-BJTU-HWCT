...
{
    rt_ubase_t sum = 0;
    for (rt_ubase_t j = 0 ;j < 10000; j ++) {
        for (rt_ubase_t i = 0 ;i < 100000000; i ++) {
            sum = sum + i;
            if ((i + 1) % 100 == 0) rt_kprintf("add 1 to %d, sum=%d\n", i, sum);
        }
    }
    
}
FINSH_FUNCTION_EXPORT(simple_add, execute simple add program);
MSH_CMD_EXPORT(simple_add, execute simple add program);

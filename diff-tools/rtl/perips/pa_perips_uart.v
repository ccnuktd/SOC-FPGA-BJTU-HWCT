...
`ifdef TESTBENCH_VCS
assign uart_tx_clk_cycle[`DATA_BUS_WIDTH-1:0]    = (`XTAL_FREQ_HZ) / UART_BAUD;
assign uart_rx_clk_cycle[`DATA_BUS_WIDTH-1:0]    = (`XTAL_FREQ_HZ) / UART_BAUD;
assign uart_rx_clk_cycle_d2[`DATA_BUS_WIDTH-1:0] = (`XTAL_FREQ_HZ) / UART_BAUD / 2;
`else
assign uart_tx_clk_cycle[`DATA_BUS_WIDTH-1:0]    = (`XTAL_FREQ_HZ / 2) / UART_BAUD;
assign uart_rx_clk_cycle[`DATA_BUS_WIDTH-1:0]    = (`XTAL_FREQ_HZ / 2) / UART_BAUD;
assign uart_rx_clk_cycle_d2[`DATA_BUS_WIDTH-1:0] = (`XTAL_FREQ_HZ / 2) / UART_BAUD / 2;
`endif
...
// Debug: Direct $write to stdout for UART TX (for Verilator simulation)
// This prints characters immediately when TX register is written

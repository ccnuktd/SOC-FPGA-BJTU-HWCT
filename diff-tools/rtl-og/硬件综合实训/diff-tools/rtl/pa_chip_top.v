...
assign clk_50m = clk_50m_t;
assign clk_out = clk_50m_t;

`endif // `ifdef TESTBENCH_VCS
...
reg [31:0] led_out_reg;
always @ (posedge clk_50m or negedge rst_n) begin
    if (!rst_n) begin
        led_out_reg <= 32'b0;
    end else if (s6_we) begin
        led_out_reg <= m1_wdata[31:0];
    end
end
...
// pa_perips_tcm u_pa_perips_ram_iap (
//     .clk_i                              (clk_50m),
//     .rst_n_i                            (rst_n),
...
assign s1_data[`DATA_BUS_WIDTH-1:0] = 32'b0;
